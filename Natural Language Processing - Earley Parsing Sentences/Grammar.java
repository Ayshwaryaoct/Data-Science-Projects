...
public class Grammar
{
	...
	public Grammar(String f) throws Exception
	{
		...
		while(str!=null)
		{
			...
			//we convert the probability to logarithim probability
...
//		System.out.println("attach:"+p.toString());
		for(int j=0;j<col.size();j++)
		{
			...
			if(q.right.length > q.dot)
				...
				{	//Attach!
					...
					//At this point when we add the completed/semicompleted production r
					//to the current column, we also add a link of backtrack to the production p
					//that was used to perform this whole step. 
					//So later we can retrieve the complete parsed tree.
					//There is no problem in adding backtrack to productions that won't
					//be utilized in the future, because in the end we only return the correct parse
					//and the non-used production and links are deleted by the "garbage collector"
					
					
					r.backtrack.add(p);

					//Here we summate the logarithmic probabilities of each rule we complete
	...
	{
		//This is a vector of all the vectors which stores the columns of the table
		...
		for(int pos=0;pos<=sent.length;pos++)
		{
			...
			int i=0;
			boolean match = false;
			//Checks through the whole vector, even as it gets bigger
			while(i!=cur.size())
			{
			...
			//check duplicated with higher probabilities
	...
	private final void addProd(Vector v, Production p)
	{
		...
		for(int i=0;i<v.size();i++) {
			...
			//To tackle the confusion we have to allow
			//adding two equal productions with different probabilities
			//With the method deleteDuplicated we examine the column
